...
class NotificationManager:
    ...
    def add_stopover_message(self,flight):
        if(flight.stopovers > 0):
            return f"\n\nFlight has {flight.stopovers} stop over, via  {flight.stopovername}"
    def send(self,flight):
        ...
        with smtplib.SMTP(self.EMAIL_DATA["Host"]) as smtp:
            ...
            msg=f"To: {self.EMAIL_DATA['ToEmail']}\nSubject: Cheap flight to {flight.city_to}\n\nLow price alert! Only £{flight.price} to fly from {flight.city_to}-{flight.to_code} to {flight.city_from}-{flight.from_code} , from {flight.from_date.strftime('%d/%m/%Y')} to {flight.to_date.strftime('%d/%m/%Y')}.{self.add_stopover_message(flight)}".encode("ascii","ignore").decode("ascii"))